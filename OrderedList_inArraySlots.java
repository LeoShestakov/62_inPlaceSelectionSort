...
public class OrderedList_inArraySlots
    ...
    public OrderedList_inArraySlots
		...
		for (int i = 0; i < unordered.size(); i++) {
			nextLargerAt = champIndex(unordered, i);
			saved = unordered.get(i);
			savedIndex = nextLargerAt;
			
			unordered.set(i, unordered.get(nextLargerAt));
			unordered.set(savedIndex, saved);
		}
    ...
    /** 
      ...
      @return ??
     */
    private int champIndex( ArrayList<Integer> challengers, int minIndex) {
		Integer champ = Integer.MAX_VALUE;
		int index = 0;
		for (int i = minIndex; i < challengers.size(); i++){
